...
- **Storybook**: UI 컴포넌트 문서 및 카탈로그 (106개 스토리)
...
| Storybook | UI 컴포넌트 문서 (106개 스토리, 97% 커버리지) | https://hchat-wiki-storybook.vercel.app | 6006 |
...
- **@hchat/tokens**: 모든 앱에서 CSS 변수로 import (155개 디자인 토큰, light + dark)
- **@hchat/ui**: 공유 컴포넌트를 경로별로 export (92개 컴포넌트)
...
### Storybook
- 모든 UI 컴포넌트의 인터랙티브 카탈로그 (106개 스토리, 97% UI 커버리지)
- 다크 모드 테마 toggle
- 접근성 검사 (WCAG 2.1 AA)
- 컴포넌트별 Props 문서
...
테스트 파일: `packages/ui/__tests__/*.test.{ts,tsx}` (13개 파일, 182 테스트)
...
│       └── stories/          # 스토리 정의 (104개 스토리, 10개 카테고리: Wiki, Admin, User, ROI, HMG, LLM Router, Shared, Atoms, Molecules, Organisms, Design Tokens)
│
├── packages/
│   ├── tokens/
│   │   └── styles/
│   │       └── tokens.css    # 디자인 토큰 CSS 변수 (155개, light + dark)
│   │
│   └── ui/
│       └── src/
...
- **단위 테스트**: Vitest 4 + @testing-library/react 16 (13파일, 182 테스트)
- **E2E 테스트**: Playwright 1.58.2 (18파일, 728줄)

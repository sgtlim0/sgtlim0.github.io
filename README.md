...
- **Storybook**: UI 컴포넌트 문서 및 카탈로그 (155 스토리 파일)
...
| TS/TSX 파일 | **1,114개** (ui 400 + apps 714) |
...
| 서비스 파일 | 39개 |
| 페이지 | 55개 (page.tsx) |
| 커스텀 훅 | 42 파일 (shared 36 + user 5 + roi 1) |
| Storybook | 155 파일 (26 interaction tests) |
| CSS 디자인 토큰 | 194개 (light + dark) |
| 단위 테스트 | 192 파일, 4,306 테스트 (Vitest) |
| 테스트 커버리지 | 89.9% stmts, 80.3% branches, 90.2% funcs |
...
| Storybook | UI 컴포넌트 문서 (155 스토리 파일) | https://hchat-storybook.vercel.app | 6006 |
...
@hchat/tokens (194 CSS 변수)
    ↓
@hchat/ui (400 파일, 108+ 컴포넌트, 42 훅 파일, 39 서비스)
    ↓
    ├── @hchat/wiki
    ├── @hchat/hmg
    ├── @hchat/admin
...
- **@hchat/ui**: 공유 컴포넌트를 경로별로 export (108+ 컴포넌트)
...
- 155 스토리 파일 (Admin 44, User 33, ROI 26, Wiki 13, HMG 12, Mobile 8, LLM Router 7, Desktop 6, Shared 5, Design System 1)
- 다크 모드 테마 toggle
- 접근성 검사 (WCAG 2.1 AA)
...
192개 파일, 4,306 테스트 (89.9% stmts, 80.3% branches, 90.2% funcs): 공통 컴포넌트, 유틸리티, 서비스 레이어, 훅, 스키마 검증, Feature Flags, 구조화 로깅, Web Worker, API 버저닝, 오프라인 큐, 성능 최적화
...
**Last Updated**: 2026-03-10 | **Phase**: 89 완료 | **Monorepo Version**: 0.1.0

...
- **Storybook**: UI 컴포넌트 문서 및 카탈로그 (204 스토리 파일)
...
| TS/TSX 파일 | **1,225개** (ui 490 + apps 735) |
...
| Storybook | 204 파일 (31+ interaction tests) |
| CSS 디자인 토큰 | 194개 (light + dark) |
| 단위 테스트 | **235 파일, 5,821 테스트** (Vitest) |
| 테스트 커버리지 | **89.24% stmts, 81.2% branches, 89.8% funcs, 90.25% lines** |
...
| Storybook | UI 컴포넌트 문서 (204 스토리 파일) | https://hchat-storybook.vercel.app | 6006 |
...
@hchat/tokens (194 CSS 변수)
    ↓
@hchat/ui (490 파일, 190 컴포넌트, 69 훅 파일, 48 서비스)
    ↓
    ├── @hchat/wiki
    ├── @hchat/hmg
    ├── @hchat/admin
...
235개 파일, 5,821 테스트 (89.24% stmts, 81.2% branches, 89.8% funcs, 90.25% lines): 공통 컴포넌트, 유틸리티, 서비스 레이어, 훅, 스키마 검증, Feature Flags, 구조화 로깅, Web Worker, API 버저닝, 오프라인 큐, 성능 최적화, Event Bus, Request Dedup, Playground
...
- **[FINAL_MILESTONE.md](docs/FINAL_MILESTONE.md)** -- Phase 74->100 전체 여정 및 마일스톤
...
**Last Updated**: 2026-03-11 | **Phase**: 100 완료 | **Workers**: 100+ | **Tests**: 5,821 | **Monorepo Version**: 0.1.0

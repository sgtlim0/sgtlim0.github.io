...
- **Storybook**: UI 컴포넌트 문서 및 카탈로그 (167 스토리 파일)
...
| 단위 테스트 | 154 파일, 3,223 테스트 (Vitest) |
| 테스트 커버리지 | 89.9% stmts, 81.4% branches |
| E2E 테스트 | 20 파일 (Playwright) |
| Interaction Tests | 51 파일 (Storybook play) |
...
│   └── ui/              # @hchat/ui — 공유 UI 컴포넌트 (~37,800줄, 321파일)
│       └── src/
...
| Storybook | UI 컴포넌트 문서 (167 스토리 파일) | https://hchat-storybook.vercel.app | 6006 |
...
@hchat/tokens (194 CSS 변수)
    ↓
@hchat/ui (~37,800줄, 154 컴포넌트, 12 훅 파일)
    ↓
    ├── @hchat/wiki
    ├── @hchat/hmg
    ├── @hchat/admin
...
- **@hchat/ui**: 공유 컴포넌트를 경로별로 export (154개 컴포넌트)
...
- 167 스토리 파일 (Admin, ROI, User, HMG, LLM Router, Desktop, Mobile, Shared, atoms/molecules/organisms)
- 다크 모드 테마 toggle
- 접근성 검사 (WCAG 2.1 AA)
...
154개 파일, 3,223 테스트 (89.9% stmts, 81.4% branches): 공통 컴포넌트, 유틸리티, 서비스 레이어, 훅, 스키마 검증, Feature Flags, 구조화 로깅, Web Worker, API 버저닝
...
20개 파일: 페이지 렌더링, 다크모드, 반응형, 접근성 (axe-core WCAG 2.1 AA), 크로스앱 네비게이션, 에러 경로, 복원력
...
**Last Updated**: 2026-03-08 | **Phase**: 83 완료 | **Monorepo Version**: 0.1.0

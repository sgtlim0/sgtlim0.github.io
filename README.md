...
- **Mobile**: 모바일 AI 챗 앱 (스와이프 제스처, 반응형 UI)
- **Storybook**: UI 컴포넌트 문서 및 카탈로그 (151개 스토리)
...
| Mobile | 모바일 AI 챗 (스와이프 제스처, 반응형) | https://hchat-mobile.vercel.app | 3005 |
| Storybook | UI 컴포넌트 문서 (151개 스토리) | https://hchat-wiki-storybook.vercel.app | 6006 |
...
- **@hchat/tokens**: 모든 앱에서 CSS 변수로 import (194개 디자인 토큰, light + dark)
...
- 실시간 모니터링 대시보드 (라이브 차트, 메트릭 카드, 활동 피드, 모델 분포)
- 대시보드 (메트릭, 사용량)
...
- SSE 스트리밍 플레이그라운드 (토큰 단위 실시간 응답, 프로바이더별 지연 프로파일)
- 모델 비교 (2-3개 모델 나란히 비교, 가격/성능 시각화)
- API 키 관리 + 보안 유틸리티 (마스킹, 검증, 생성, 비용 추정)
- 11개 페이지 인터페이스
- 대시보드 및 통계

### Desktop (@hchat/desktop)
- 데스크톱 AI 챗 인터페이스 (Next.js 16, 모노레포 통합)
- DesktopSidebar (접이식 네비게이션), DesktopChatBubble (토큰 카운트)
- AgentCard (에이전트 상태/제어), ToolGrid (도구 활성/비활성 상태)
- 24개 전용 디자인 토큰 (`--dt-*` 접두사)
- 개발 서버: localhost:5173

### Mobile (@hchat/mobile)
- 모바일 AI 챗 앱 (Next.js 16, 모노레포 통합)
- MobileApp, MobileChatList, 스와이프 제스처 UI
- 터치 최적화 반응형 디자인
- 개발 서버: localhost:3005
...
### Storybook
- 모든 UI 컴포넌트의 인터랙티브 카탈로그 (151개 스토리)
- 다크 모드 테마 toggle
- 접근성 검사 (WCAG 2.1 AA)
- 컴포넌트별 Props 문서
...
23개 이상의 페이지에 `next/dynamic`을 적용하여 초기 번들 크기를 최적화합니다:
...
Vitest 4 + @testing-library/react 16 기반 단위 테스트:
...
# 커버리지 리포트 (v8, lcov, html)
npm run test:coverage
```

테스트 파일: `packages/ui/__tests__/*.test.{ts,tsx}` (20개 파일, 259 테스트)

테스트 대상:
- 공통 컴포넌트: Badge, EmptyState, Skeleton, FeatureCard
- 유틸리티: validation (validate, useFormValidation, patterns)
- 서비스 레이어: Admin MockApi, User chatService, LLM Router MockService, ROI aggregateData
- 실시간 서비스: realtimeService (구독/해제), realtimeHooks (메트릭/시계열/활동 훅)
...
### HMG / Admin / User / LLM Router / Desktop / Mobile / Storybook (Vercel)
- Vercel Git 연동 자동 배포
- 각 앱별 독립적인 배포 설정
- URLs:
...
| 페이지 | 55개 (page.tsx 기준) |
| Dynamic Import 페이지 | 23+개 |
| Storybook 스토리 | 151개 |
| CSS 디자인 토큰 | 194개 (light + dark) |
| 커스텀 훅 | 58개 (Admin 29, User 7, LLM Router 8+) |
| 단위 테스트 | 20개 파일 (Vitest 4, 259 테스트) |
| E2E 테스트 파일 | 18개 (Playwright) |
...
| 패키지/앱 | 코드 라인 |
|---|---|
| packages/ui/src | 23,385 |
| apps/storybook | 5,809 |
| apps/llm-router | 2,317 |
| apps/wiki | 1,390 |
| apps/desktop | 829 |
| apps/admin | 674 |
| apps/hmg | 569 |
...
npm run dev:desktop       # Desktop 개발 서버 (localhost:5173)
npm run dev:mobile        # Mobile 개발 서버 (localhost:3005)
...
npm run build:desktop     # Desktop만 빌드
npm run build:mobile      # Mobile만 빌드
...
│       └── stories/          # 스토리 정의 (151개 스토리: Admin 31, ROI 24, User 21, HMG 12, LLM Router 7, Mobile 6, Desktop 6, Shared 5 등)
│
├── packages/
│   ├── tokens/
│   │   └── styles/
│   │       └── tokens.css    # 디자인 토큰 CSS 변수 (194개, light + dark)
│   │
│   └── ui/
│       └── src/
...
- **단위 테스트**: Vitest 4 + @testing-library/react 16 (20파일, 259 테스트)
- **E2E 테스트**: Playwright 1.58.2 (18파일)
- `playwright.config.ts`에서 프로젝트별 구성
- `vitest.config.ts`에서 모노레포 alias 설정

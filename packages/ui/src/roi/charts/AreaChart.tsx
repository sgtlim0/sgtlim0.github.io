...
export default function AreaChart({ data, color = 'var(--roi-chart-3)', height = 200 }: AreaChartProps) {
  ...
  const areaPoints = `${getX(0)},${bottomY} ${linePoints} ${getX(data.length - 1)},${bottomY}`;

  return (
    <div className="relative w-full" role="img" aria-label="에어리어 차트">
      <svg viewBox={`0 0 100 ${height}`} className="w-full" preserveAspectRatio="none" style={{ height }}>
        ...
        {/* Line */}
        <polyline fill="none" stroke={color} strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" points={linePoints} vectorEffect="non-scaling-stroke" />
        {/* Dots */}
        {data.map((d, i) => (
          <circle
            key={i}
      ...
      {hovered !== null && (
        ...
        >
          {data[hovered].label}: ₩{data[hovered].value.toLocaleString()}M
        </div>
      )}
    </div>
  );

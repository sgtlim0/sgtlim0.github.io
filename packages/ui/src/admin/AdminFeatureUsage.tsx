...
export default function AdminFeatureUsage() {
  ...
  const maxWeekly = Math.max(...WEEKLY_TREND.map((w) => w.chat + w.groupChat + w.toolUse + w.agent + w.debate + w.report));
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="가장 인기 기능" value={topFeature.name} trend={`${topFeature.usagePercentage}%`} trendUp />
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-admin-bg-section">
                <th className="text-left py-3 px-4 text-xs font-medium text-text-secondary">기능명</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">월간 사용량</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">전월 대비</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">활성 사용자</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">평균 응답시간</th>
                <th className="text-center py-3 px-4 text-xs font-medium text-text-secondary">만족도</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-admin-border">
              {FEATURES.map((feature) => (
                <tr key={feature.name} className="hover:bg-admin-bg-hover transition-colors">
                  <td className="py-3 px-4 text-sm font-medium text-text-primary">{feature.name}</td>
                  <td className="py-3 px-4 text-sm text-text-primary text-right tabular-nums">{feature.monthlyUsage}</td>
                  <td className="py-3 px-4 text-sm text-right">
                    <span className={`font-medium ${feature.changeUp ? 'text-admin-status-success' : 'text-admin-status-error'}`}>
                      {feature.changeUp ? '↑' : '↓'} {feature.changePercent}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-sm text-text-primary text-right tabular-nums">{feature.activeUsers}</td>
                  <td className="py-3 px-4 text-sm text-text-secondary text-right tabular-nums">{feature.avgResponseTime}</td>
      ...
      <div className="rounded-xl bg-admin-bg-card border border-admin-border p-5">
          ...
          {WEEKLY_TREND.map((w) => {
            ...
            return (
              <div key={w.week} className="flex items-center gap-4">
                <span className="text-sm text-text-secondary w-12 shrink-0 tabular-nums">{w.week}</span>
        ...
        <div className="flex items-center flex-wrap gap-6 mt-4 pt-3 border-t border-admin-border">
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-admin-teal" /><span className="text-xs text-text-secondary">Chat</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#3B82F6]" /><span className="text-xs text-text-secondary">Group Chat</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#8B5CF6]" /><span className="text-xs text-text-secondary">Tool Use</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#F59E0B]" /><span className="text-xs text-text-secondary">Agent</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#EF4444]" /><span className="text-xs text-text-secondary">Debate</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded bg-[#10B981]" /><span className="text-xs text-text-secondary">Report</span></div>
        </div>
      </div>
          ...
          {ADOPTION_RATE.map((item) => (
            <div key={item.name} className="p-4 rounded-lg bg-admin-bg-section">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-text-primary tabular-nums">{item.rate}%</span>
              </div>

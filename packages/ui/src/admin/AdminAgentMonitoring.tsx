...
const statusConfig = {
  running: { label: '실행 중', color: 'bg-[#10B981]', textColor: 'text-[#10B981]', bgColor: 'bg-[#10B981]/10' },
  idle: { label: '대기', color: 'bg-[#3B82F6]', textColor: 'text-[#3B82F6]', bgColor: 'bg-[#3B82F6]/10' },
  error: { label: '오류', color: 'bg-[#EF4444]', textColor: 'text-[#EF4444]', bgColor: 'bg-[#EF4444]/10' },
};
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {AGENTS.map((agent) => {
          ...
          return (
            <div key={agent.name} className="p-5 rounded-xl bg-admin-bg-card border border-admin-border">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-bold text-text-primary">{agent.name}</h3>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${status.textColor} ${status.bgColor}`}>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-text-primary tabular-nums">{agent.totalExecutions.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-text-secondary">성공률</span>
                  <span className="font-medium text-text-primary tabular-nums">{agent.successRate}</span>
                </div>
              </div>
            </div>
          );
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-admin-bg-section">
                <th className="text-left py-3 px-4 text-xs font-medium text-text-secondary">시간</th>
                <th className="text-left py-3 px-4 text-xs font-medium text-text-secondary">에이전트</th>
                <th className="text-left py-3 px-4 text-xs font-medium text-text-secondary">작업</th>
                <th className="text-center py-3 px-4 text-xs font-medium text-text-secondary">상태</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">실행 시간</th>
                <th className="text-right py-3 px-4 text-xs font-medium text-text-secondary">토큰 사용</th>
              </tr>
            </thead>
              ...
              {EXECUTIONS.map((exec, i) => {
                ...
                return (
                  <tr key={i} className="hover:bg-admin-bg-hover transition-colors">
                    <td className="py-3 px-4 text-sm text-text-secondary tabular-nums">{exec.timestamp}</td>
                    <td className="py-3 px-4 text-sm font-medium text-text-primary">{exec.agentName}</td>
                    <td className="py-3 px-4 text-sm text-text-secondary">{exec.task}</td>
                    <td className="py-3 px-4 text-center">
                      <span className={`inline-flex items-center gap-1.5 ${execStatus.color} text-sm`}>
                        <span className={`w-2 h-2 rounded-full ${execStatus.dot}`} />
                        {exec.status === 'success' ? '성공' : exec.status === 'failed' ? '실패' : '실행 중'}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-sm text-text-primary text-right tabular-nums">{exec.duration}</td>
                    <td className="py-3 px-4 text-sm text-text-secondary text-right tabular-nums">{exec.tokenUsage}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          {DAILY_TREND.map((d) => {
            ...
            return (
              <div key={d.time} className="flex items-center gap-4">
                <span className="text-sm text-text-secondary w-16 shrink-0 tabular-nums">{d.time}</span>

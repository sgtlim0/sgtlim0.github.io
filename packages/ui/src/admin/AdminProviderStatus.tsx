...
const INCIDENT_LOG = [
  { time: '2026-03-03 14:22', provider: 'Google Gemini', event: '응답 지연 발생 (>500ms)', severity: 'warning' as const },
  { time: '2026-03-03 09:15', provider: 'Amazon Bedrock', event: '정상 복구', severity: 'success' as const },
  { time: '2026-03-02 22:40', provider: 'Amazon Bedrock', event: '일시적 타임아웃 (5건)', severity: 'warning' as const },
  { time: '2026-03-01 16:00', provider: 'OpenAI', event: 'Rate limit 임계치 도달 (80%)', severity: 'info' as const },
];

const statusConfig = {
  online: { label: '정상', color: 'bg-[#10B981]', textColor: 'text-[#10B981]', bgColor: 'bg-[#10B981]/10' },
  degraded: { label: '지연', color: 'bg-[#F59E0B]', textColor: 'text-[#F59E0B]', bgColor: 'bg-[#F59E0B]/10' },
  offline: { label: '장애', color: 'bg-[#EF4444]', textColor: 'text-[#EF4444]', bgColor: 'bg-[#EF4444]/10' },
};
    ...
    <div className="flex flex-col gap-6 p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {PROVIDERS.map((provider) => {
          ...
          return (
            <div key={provider.name} className="p-5 rounded-xl bg-admin-bg-card border border-admin-border">
              ...
              <div className="flex flex-col gap-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-text-primary tabular-nums">{provider.latency}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-text-secondary">가동률 (30일)</span>
                  <span className="font-medium text-text-primary tabular-nums">{provider.uptime}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="pt-3 border-t border-admin-border">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {provider.models.map((model) => (
                      <span key={model} className="px-2 py-0.5 text-xs rounded bg-admin-bg-section text-text-secondary">
                        {model}
                      </span>
                    ))}
                  </div>
          ...
          {INCIDENT_LOG.map((log, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-4 px-5 py-3">
                ...
                <span className="text-xs text-text-secondary tabular-nums w-36 shrink-0">{log.time}</span>
                <span className="text-sm font-medium text-text-primary w-32 shrink-0">{log.provider}</span>

...
export default function AdminModelPricing() {
  ...
  const totalCost = totalMonthly
    ? totalMonthly.bedrock + totalMonthly.openai + totalMonthly.google
    : 0
              ...
              {MODELS.map((model) => {
                ...
                return (
                  <tr key={model.name} className="hover:bg-admin-bg-hover transition-colors">
                    ...
                    <td className="py-3 px-4 text-sm text-text-secondary text-center tabular-nums">
                      {model.contextWindow}
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-semibold ${tier.color} ${tier.bg}`}
                      >
                        {tier.label}
                      </span>
                    </td>
      ...
      <div className="rounded-xl bg-admin-bg-card border border-admin-border p-5">
          ...
          {MONTHLY_COST.map((m) => {
            ...
            return (
              <div key={m.month} className="flex items-center gap-4">
                <span className="text-sm text-text-secondary w-20 shrink-0 tabular-nums">
                  {m.month}
                </span>
        ...
        <div className="flex items-center gap-6 mt-4 pt-3 border-t border-admin-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-text-secondary">Bedrock</span>
          </div>
          <div className="flex items-center gap-2">

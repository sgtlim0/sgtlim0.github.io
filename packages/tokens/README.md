...
packages/tokens/
├── styles/
│   └── tokens.css      # 155 CSS 변수 (light/dark)
├── src/
...
- **Vitest 4**: 11개 테스트 파일
- **커버리지**: @vitest/coverage-v8 (v8, lcov, html 리포트)
- **CI 통합**: GitHub Actions ci.yml에 `npm test` 단계 추가

### Phase 29: 실시간 대시보드
- **실시간 컴포넌트**: LiveMetricCard, LiveLineChart, LiveActivityFeed, LiveModelDistribution
- **서비스 레이어**: Mock setInterval 기반 실시간 데이터 스트리밍
- **Admin 토큰 활용**: `--admin-*` CSS 변수로 라이브 차트 스타일링

...
**레이아웃**:
- 상단 GNB: H Chat 로고 + 탭 메뉴 (업무 비서, 문서 번역, 문서 작성, 텍스트 추출) + 사용자 이메일 + 기업용 버전 가입 + 언어 선택
...
| 업무 대화 | 자연어 질의 → AI 응답 |
| 비서 탭 | "H Chat 공식 비서" (플랫폼 제공) / "내가 만든 비서" (커스텀) |
...
├── AssistantTabs ("H Chat 공식 비서" | "내가 만든 비서")

...
멀티턴 에이전트는 H Chat v4에서 강화된 기능으로, AI가 여러 단계에 걸쳐 자율적으로 도구를 사용하며 복잡한 작업을 수행할 수 있습니다.
...
| **timestamp_convert** | 타임스탬프 변환 | `timestamp`, `format` | Unix 타임스탬프 변환 및 형식화 |
| **translate_text** | 텍스트 번역 | `text`, `from`, `to` | 50개+ 언어 간 번역 |
| **summarize_content** | 콘텐츠 요약 | `content`, `length` | 긴 텍스트의 핵심 요약 생성 |
| **other** | 기타 작업 | (가변) | 커스텀 플러그인 실행 (Webhook, JavaScript 등) |
...
Unix 타임스탬프를 사람이 읽을 수 있는 형식으로 변환하거나, 날짜를 타임스탬프로 변환합니다.

```xml
<tool_call>
  <name>timestamp_convert</name>
  <params>{"timestamp": 1709251200000, "format": "YYYY-MM-DD HH:mm:ss"}</params>
</tool_call>
```

**반환**: 변환된 날짜/시간 문자열 또는 타임스탬프

#### 6. translate_text

텍스트를 다른 언어로 번역합니다. 50개 이상의 언어를 지원합니다.

```xml
<tool_call>
  <name>translate_text</name>
  <params>{"text": "Hello world", "from": "en", "to": "ko"}</params>
</tool_call>
```
...
```xml
<tool_call>
  <name>summarize_content</name>
  <params>{"content": "...", "length": "short"}</params>
</tool_call>
```

**반환**: 요약된 텍스트 (short/medium/long)

#### 8. other

커스텀 플러그인을 실행합니다. Webhook 호출, JavaScript 코드 실행, 프롬프트 템플릿 적용 등 다양한 작업을 수행할 수 있습니다.

```xml
<tool_call>
  <name>other</name>
  <params>{"plugin": "webhook", "action": "send", "data": {...}}</params>
</tool_call>
```
...
v4에서는 사용자 정의 플러그인을 통해 에이전트 기능을 확장할 수 있습니다.

### Webhook 플러그인

외부 API와 통신하여 데이터를 가져오거나 전송합니다.
...
안전한 샌드박스 환경에서 JavaScript 코드를 실행합니다.
...
<tool_call>
  ...
  <params>{
    "plugin": "javascript",
    "code": "const data = [1,2,3]; return data.reduce((a,b) => a+b, 0);"
  }</params>
</tool_call>
```
...
미리 정의된 프롬프트 템플릿을 적용하여 일관된 출력을 생성합니다.
...
<tool_call>
  ...
  <params>{
    "plugin": "template",
    "template": "email_formal",
    "variables": {"recipient": "홍길동", "subject": "회의 일정"}
  }</params>
</tool_call>
```
